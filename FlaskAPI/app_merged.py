...
# India Model
try:
    india_model_path = os.path.join(current_dir, '..', 'india_salary_prediction', 'india_ds_model.joblib')
    india_columns_path = os.path.join(current_dir, '..', 'india_salary_prediction', 'india_model_columns.joblib')
...
def extract_skills(text):
    ...
    skills_db = {
        'python': 'Python', 'sql': 'SQL', 'aws': 'AWS', 'excel': 'Excel',
        'machine learning': 'Machine Learning', 'deep learning': 'Deep Learning',
        'spark': 'Spark', 'hadoop': 'Hadoop', 'tableau': 'Tableau', 'power bi': 'Power BI',
        'sas': 'SAS', 'java': 'Java', 'c++': 'C++', 'r': 'R'
    }
    found_skills = []
...
def explain_us():
    try:
        ...
        # For regression, shap_values is just an array, or list of arrays?
        # Typically array of shape (1, n_features)
        
        # Simplify response: return feature importance values
        # Assume feature names are known or just return indices
        feature_names = [
            "Rating", "Size", "Type of ownership", "Industry", "Sector", "Revenue", "num_comp", "hourly", 
            "employer_provided", "job_state", "same_state", "age", "python_yn", "spark", "aws", "excel", 
            "job_simp", "seniority", "desc_len"
        ] # Placeholder names based on common columns, better to verify from model if possible
