...
def predict():
    ...
    try:
        ...
        return jsonify(
            create_prediction_response(
                prediction, currency="USD (Thousands Per Annum)", market="US"
            )
        )
...
def explain():
    ...
    try:
        ...
        explanation = [{"feature_index": i, "shap_value": float(val)} for i, val in enumerate(sv)]
        ...
        return jsonify(
            {
                ...
                "base_value": (
                    float(us_explainer.expected_value)
                    if hasattr(us_explainer, "expected_value")
                    else None
                ),
            }
        )
